...
AnalizadorPresion::AnalizadorPresion() {
    nombre = "Analizador de Presión";
}
...
void AnalizadorPresion::procesarLineaDatos(const std::string& linea, const Vector3& pos) {
    ...
    if (inside) {
        double presion_fix = -(sxx - m * vx * vx + syy - m * vy * vy + szz - m * vz * vz) / 3.0;
...
void AnalizadorPresion::guardarResultados(const std::string& filepath) {
    std::string outname = "presion_" + fs::path(filepath).filename().string() + ".dat";
